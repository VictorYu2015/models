...
class TfExampleDecoder(data_decoder.DataDecoder):
  ...
  def _reshape_keypoint_visibilities(self, keys_to_tensors):
    ...
    The returned boolean is only True for the labeled and visible case.
    ...
    Returns:
      A 2-D bool tensor of shape [num_instances, num_keypoints] with values
        in {0, 1}. 1 if the keypoint is labeled and visible, 0 otherwise.
    """
    x = keys_to_tensors['image/object/keypoint/x']
    ...
    if isinstance(x, tf.SparseTensor):
      x = tf.sparse_tensor_to_dense(x)
    def _default_visibilities():
      return Visibility.VISIBLE.value * tf.ones_like(x, dtype=tf.int64)
    vis = tf.cond(tf.equal(tf.size(x), tf.size(vis)),
                  true_fn=lambda: vis,
                  false_fn=_default_visibilities)
    vis = tf.equal(vis, Visibility.VISIBLE.value)
