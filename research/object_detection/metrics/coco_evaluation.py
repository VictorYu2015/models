...
class CocoKeypointEvaluator(CocoDetectionEvaluator):
  ...
  def __init__(self,
    ...
    Args:
      categories: A list of dicts, each of which has the following key:
        'id': (required) an integer id uniquely identifying this category.
        'name': (required) string representing category name e.g., 'cat', 'dog'.
      category_keypoints: A dict specifying the category to keypoint mappings:
        key = a string specifying the category name.
        value = a list of dicts each of which contains:
      ...
      oks_sigmas: A dict of keypoint name to variance value for OKS
        metrics. If not provided, default value of 0 is assumed for keypoints.
    """
    self._category_id = [
        category['id']
        for category in categories
        if category['name'] == class_text
    ]
    self._category_id = self._category_id[0]
    self._category_name = class_text
    self._keypoints = category_keypoints[class_text]
    self._keypoint_ids = [keypoint['id'] for keypoint in self._keypoints]
    self._oks_sigmas = oks_sigmas
    if self._oks_sigmas is None:
      self._oks_sigmas = np.full((len(self._keypoints)), 0)
    else:
      self._oks_sigmas = [
          oks_sigmas[keypoint['name']] for keypoint in self._keypoints
      ]
    self._metrics = None
  ...
  def clear(self):
    ...
    super(CocoKeypointEvaluator, self).clear()
  ...
  def add_eval_dict(self, eval_dict):
    ...
    def update_op(
      ...
      for (image_id, gt_box, gt_class, gt_is_crowd, gt_keyp, gt_keyp_vis,
           num_gt_box, det_box, det_score, det_class, det_keyp,
    ...
    return tf.py_func(update_op, [
        ...
        num_gt_boxes_per_image, detection_boxes, detection_scores,
        detection_classes, detection_keypoints, num_det_boxes_per_image,
        is_annotated
    ], [])
